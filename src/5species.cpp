...
inline void write_0D_header(const std::string& filename,
    ...
    out << "t,T_tr,T_v,E,Ev,R_mix,rho,p";

    for (const auto& nm : RS.species) 
        out << ",X_" << nm.name;

    out << "\n";
}   

inline void append_0D_row(const std::string& filename,
                          double t,
...
int main() {
    ...
    while (t < 1.0e-6) {
        ...
        landau_teller(Q_vt, rxns, Ts, rho_s.data(), Ys, Xs, P);
        chemical_vibrational(Q_cv, rxns, Ts[1], rates);

        if (counter % NWRITE == 0) {
            append_0D_row(filename, t, Ts, E, Ev, Xs.data(), rho, P, R_mix, rxns.n_species);
        }
